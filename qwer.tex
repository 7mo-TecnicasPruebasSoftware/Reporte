...
\author{Saul Lopez Mendoza \\ Bryan Miguel Amador Castillo \\ Edson Joel Carrera Avila}
\date{10/04/2025}
...
\begin{itemize}[label=--]
    \item Avance 1 (04/10/2026): Planteamiento, análisis de requisitos, casos de uso y mockups.
...
\begin{itemize}[label=--]
    \item OE1: Implementar un módulo de administración de usuarios con roles diferenciados (invitado, cliente estándar, cliente VIP, administrador).
    \item OE2: Desarrollar el catálogo de productos con organización por categorías y funcionalidad de búsqueda/filtrado accesible para usuarios no registrados.
    \item OE3: Implementar el carrito de compras con persistencia de sesión y cálculo automático de totales y descuentos.
    \item OE4: Desarrollar el flujo de pago que registre la orden y actualice el stock de cada producto vendido.
    \item OE5: Construir el panel de administración para la gestión de productos con captura en formato UPC, categorías y órdenes (altas, bajas, cambios).
    \item OE6: Aplicar técnicas de pruebas de software (partición equivalente, análisis de valores límite) sobre los módulos críticos del sistema.
\end{itemize}
...
\begin{itemize}[label=--]
    \item RN1: El correo electrónico de cada usuario debe ser único en la base de datos.
    \item RN2: Las contraseñas deben cifrarse antes de persistirse en la base de datos.
    \item RN3: Solo usuarios activos pueden iniciar sesión en el sistema.
    \item RN4: La sesión expira tras 30 minutos de inactividad.
    \item RN5: Si un usuario inicia sesión en otro dispositivo, la sesión anterior se invalida automáticamente.
    \item RN6: Si un usuario inicia sesión desde un dispositivo mientras tiene otra sesión activa, la sesión anterior se invalida automáticamente sin notificación al usuario previo.
    \item RN7: La desactivación de un usuario no elimina ni altera sus registros financieros u órdenes previas.
    \item RN8: Usuarios desactivados no pueden iniciar sesión ni realizar nuevas transacciones.
    \item RN9: La búsqueda de productos se realiza únicamente sobre productos activos.
    \item RN10: Cada categoría debe tener un nombre único.
    \item RN11: La baja de categorías es lógica (estado inactivo), no física.
    \item RN12: Categorías dadas de baja pueden ser reactivadas.
    \item RN13: La baja lógica de categorías no elimina ni altera referencias históricas de productos asociados.
    \item RN14: Cada producto debe tener un código UPC único.
    \item RN15: La baja de productos es lógica (estado inactivo), no física.
    \item RN16: Productos dados de baja pueden ser reactivados.
    \item RN17: La baja lógica de productos no elimina ni altera órdenes históricas.
    \item RN18: Un cliente no puede agregar más unidades al carrito que las disponibles en stock.
    \item RN19: El límite máximo de unidades por producto en el carrito es el menor valor entre el stock disponible y 10 unidades.
    \item RN20: El carrito debe pertenecer al cliente autenticado.
    \item RN21: No se pueden confirmar órdenes con carrito vacío.
    \item RN22: El stock debe validarse nuevamente al momento de confirmar una orden.
    \item RN23: El stock se reserva y descuenta únicamente al confirmar la orden.
    \item RN24: Las promociones deben tener fecha de inicio y fin claramente definidas.
    \item RN25: Se aplican promociones vigentes y beneficios VIP automáticamente.
    \item RN26: Una promoción activa de categoría tiene prioridad sobre el precio estándar del producto, pero no se acumula con el descuento VIP.
    ...
    \item RN27: Solo administradores pueden gestionar productos, categorías, usuarios y promociones.
    \item RN28: Todas las rutas administrativas deben validarse mediante middleware de autorización.
    \item RN29: Seguridad avanzada: contraseñas cifradas con algoritmos seguros (bcrypt/Argon2), limitación de intentos de login, validación y sanitización de entradas contra inyecciones.
\end{itemize}
...
\begin{enumerate}[label=FN-\arabic*]
    ...
    \item Cliente selecciona método de pago (tarjeta, PayPal, etc.).
...
\begin{tabularx}{\linewidth}{|p{2.5cm}|X|X|X|X|}
    ...
    CU10: Consultar historial de órdenes     & OE3                     & RF07                     & RN7, RN8                & RNF07, RNF08            \\ \hline
